...
export default function ProductCard({
  ...
  // Mouse tilt & parallax motion values
  ...
  // 3D Card tilt transforms (gentle realistic tilt)
  const rotateX = useTransform(smoothMouseY, [-0.5, 0.5], [6, -6]);
  const rotateY = useTransform(smoothMouseX, [-0.5, 0.5], [-6, 6]);

  // Parallax displacement for the floating transparent product
  ...
  return (
    ...
    >
      {/* ── Stable Frosted Glass Card Background ── */}
      ...
      {/* ── Top Row: Pill Badges (Faithful to Reference Image) ── */}
      ...
      <div className="relative z-10 w-full h-56 sm:h-64 my-auto flex items-center justify-center p-3">
        ...
        <motion.div
          style={{
            x: productParallaxX,
            y: productParallaxY,
            scale: isHovered ? 0.91 : 1, // Smooth "zooms out" depth effect on hover
          }}
          transition={{
        ...
        {/* Dynamic ambient hover ground shadow under the transparent product */}
        <motion.div
          animate={{
      ...
      {/* ── Bottom Row: Title + Price Left, Circular Arrow Button Right ── */}
