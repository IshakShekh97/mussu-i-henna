...
export default function Hero() {
  ...
  return (
    <section
      ref={containerRef}
      className="relative min-h-[calc(100dvh-2.2rem)] md:h-[calc(100dvh-2.5rem)] w-full overflow-hidden flex flex-col justify-between pt-12 sm:pt-14 md:pt-0 pb-0 px-3 sm:px-6 lg:px-8 mx-auto select-none"
    >
      <motion.div
        style={{
          opacity: heroOpacity,
        ...
        className="relative flex-1 flex flex-col items-center justify-between w-full my-0 py-0 origin-center transition-opacity"
      >
        {/* ── Top Giant Headline: CREATIVE HENNA ── */}
        <motion.div
          style={{ y: topTextY }}
          className="relative z-10 w-full text-center flex flex-col items-center pt-2 sm:pt-3 md:pt-2"
        >
          <motion.h1
          ...
          {/* ── Mobile Call To Action (Neatly placed, hides smoothly on scroll) ── */}
          <motion.div
        ...
        {/* ── Central Hero Hand (Responsive Sizing + Scroll-retraction Closing Effect) ── */}
        <div className="absolute inset-x-0 bottom-0 pointer-events-none flex items-end justify-center z-25">
          <motion.div
            initial={{ y: "115%", opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            style={{ y: handScrollY, scale: handScrollScale }}
            transition={{
              duration: 1.3,
              ease: [0.16, 1, 0.3, 1],
              delay: 0.12,
            }}
            className="relative h-[82vh] sm:h-[70vh] md:h-[78vh] lg:h-[86vh] max-h-240 aspect-568/1024 flex items-end justify-center origin-bottom"
          >
            <Image
              src="/hero-hand.png"
        ...
        {/* ── Left Floating Card (Glassmorphism + Exit Spread Animation) ── */}
        <motion.div
        ...
        {/* ── Right Floating Card (Bottom on Mobile, Top/Middle on Large + Exit Spread) ── */}
        <motion.div
        ...
        {/* ── Desktop Floating Magnetic Call to Action ── */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          style={{ opacity: ctaOpacity, y: ctaY }}
        ...
        <motion.div
          style={{ y: bottomTextY }}
          className="relative z-10 w-full text-center pb-1 sm:pb-2"
        >
          <motion.div
